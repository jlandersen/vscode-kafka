...
### Changed
- **BREAKING**: Updated `@faker-js/faker` to v10.
- **BREAKING**: Minimum supported VS Code version is now `1.101.0` (support for older versions has been dropped).
...
- **Explorer UX**: Show empty-state and error items in the Kafka explorer tree for topics, brokers, consumer groups, and configs.
...
- **Consumer Table View**: Added ability to view consumed Kafka messages in an Excel-like table format. Click the "View as Table" button in the consumer editor to open a table view with columns for Key, Partition, Offset, Value, and Headers. See [#109](https://github.com/jlandersen/vscode-kafka/issues/109).
...
- Deleting a consumer group now automatically stops all active consumers using that group. See [#110](https://github.com/jlandersen/vscode-kafka/issues/110).
