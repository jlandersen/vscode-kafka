...
export async function executeInlineCommand(kafkaFileDocument: KafkaFileDocument, clusterId: string, consumerLaunchStateProvider: ConsumerLaunchStateProvider) {
    ...
    if (isProducerBlock(node)) {
        ...
        const kafkaFileUri = editor?.document.uri.scheme === 'file' ? editor.document.uri : undefined;
        commandArguments = [node.createCommand(clusterId, kafkaFileUri), 1];
        ...
        const kafkaFileUri = editor?.document.uri.scheme === 'file' ? editor.document.uri : undefined;
        commandArguments = [node.createCommand(clusterId, kafkaFileUri)];
    }
