...
import { createSaslScramFixture, TestFixture } from "./kafkaContainers";
...
suite("SASL/SCRAM Integration Tests", function () {
    ...
    suite("SASL/SCRAM-SHA-256 Cluster", function () {
        ...
        suiteSetup(async function () {
            console.log("Setting up SASL/SCRAM-SHA-256 Kafka container...");
            fixture = await createSaslScramFixture();
            
            client = createTestClient(fixture.connectionInfo, "test-sasl");
            await client.connect();
        });

        suiteTeardown(async function () {
            console.log("Tearing down SASL/SCRAM-SHA-256 Kafka container...");
            if (client) {
                client.dispose();
            }
            if (fixture) {
        ...
        test("should connect to SASL/SCRAM cluster and retrieve brokers", async function () {
            ...
            console.log(`SASL/SCRAM cluster: Found ${brokers.length} broker(s):`, brokers.map(b => `${b.id}@${b.host}:${b.port}`));
        });

        test("should list topics on SASL/SCRAM cluster", async function () {
            const topics = await client.getTopics();
            
            assert.ok(topics, "Topics should be defined");
            assert.ok(Array.isArray(topics), "Topics should be an array");
            
            console.log(`SASL/SCRAM cluster: Found ${topics.length} topic(s)`);
        });

        test("should produce messages to SASL/SCRAM cluster", async function () {
            const topicName = `sasl-produce-test-${Date.now()}`;
        ...
        test("should consume messages from SASL/SCRAM cluster", async function () {
            const topicName = `sasl-consume-test-${Date.now()}`;
            const groupId = `sasl-consumer-group-${Date.now()}`;
            ...
            console.log(`Successfully consumed and verified all ${consumedMessages.length} messages from SASL/SCRAM cluster`);
